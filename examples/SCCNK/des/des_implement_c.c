...
void initP(char *pt, char *key, char *D, char* K) {
  Permute( K, key, KeyPermuteMap, 7 );
  Permute( D, pt, InitialPermuteMap, 8 );
}

void roundP(char *D, char *K, int r) {
  int    j;
...
int main(int argc, char **argv) {
  ...
  char *x, *y, *K, *D, *ct;
  int i, j;
  x = (char *) malloc(size * 8);
  y = (char *) malloc(size * 7);
  for (j = 0; j < size; j++) {
    memcpy(&x[j * 8], &pt[0], 8);
    memcpy(&y[j * 7], &key[0], 7);
  }
  K = (char *) malloc(size * 7);
  D = (char *) malloc(size * 8);
  ct = (char *) malloc(size * 8);
  
  for (j = 0; j < size; j++) {
    initP(&x[j * 8], &y[j * 7], &D[j * 8], &K[j * 7]);
    for (i = 0; i < 16; i++) {
      roundP(&D[j * 8], &K[j * 7], i);
    }
